...
const USER_AGENT = "Diogo-REST-Verify/1.2.9";
...
const [url, ...conditionParts] = parsedCli.positional;
const condicao = conditionParts.join(" ").trim();
const feedback = parsedCli.feedback;

if (!url || !condicao) {
  console.error(
    'Uso: node --env-file=.env.test .\\dist\\verify-buyer.js <URL> "<condição>" [opções de feedback]',
  );
  console.error(getFeedbackHint());
  process.exit(1);
}
...
async function main(): Promise<void> {
  try {
    ...
    currentStage = "payment";
    const response = await fetchWithPayment(VERIFY_ENDPOINT, {
      method: "POST",
      headers: { ...journeyHeaders, "Content-Type": "application/json" },
      body: JSON.stringify(requestBody),
    });
